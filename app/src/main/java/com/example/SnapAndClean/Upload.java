...
public class Upload extends AppCompatActivity implements AdapterView.OnItemSelectedListener{
    Spinner spinner;
    String[] page = {"Choose Option","Sukkha","Gilla","Stone","Plants"};
    TextView textView;
    ...
    public void uploadImage(View view){
        try {
            if(/*!uploadPicET.getText().toString().isEmpty() && */imageLocationPath != null){
                ...
                //This line sets up a reference to the location in Firebase Storage
                // where the image will be uploaded.
                final StorageReference imageRef = objectStorageReference.child(nameOfimage);

                //This line initiates the upload task for the image file (imageLocationPath)
                // to the location specified by imageRef in Firebase Storage.
                ...
                objectUploadTask.continueWithTask(new Continuation<UploadTask.TaskSnapshot, Task<Uri>>() {
                    ...
                    public void onComplete(@NonNull Task<Uri> task) {
                        if (task.isSuccessful()){
                            ...
                            //Calendar.getInstance() creates a calendar object representing the current date and time.
                            //SimpleDateFormat is used to format the date and time according to the specified pattern ("MM, dd, yyyy" for date and "HH:mm:ss a" for time).

                            //currentDate.format(calForDate.getTime()) retrieves the current date and formats it.
                            //currenttime.format(calForDate.getTime()) retrieves the current time and formats it.
                            ...
                            //uuid generates a unique identifier using UUID (Universally Unique Identifier).
                            String uuid = UUID.randomUUID().toString();
                            //objectMap is a HashMap used to store key-value pairs for Firestore document fields.
                            ...
                            objectMap.put("order","Sucessfull");
                            objectMap.put("dboy","");
                          ...
                          /*objectFirebaseFirestore.collection("Product")
                            ...
                            objectFirebaseFirestore.collection("Wastage").document(uuid)
                                    .set(objectMap)
                                    .addOnSuccessListener(new OnSuccessListener<Void>() {
                                        @Override
                                        public void onSuccess(Void unused) {
